class SquarerIter:
    ...
    def __iter__(self):
        #  чтобы объект итератора можно было использовать несколько раз, надо вернуть к исходным значения
        #  изменяющихся в процессе работы атрибутов
        self.stop_number = stop_number  # TODO это значение не изменяется в процессе работы итератора, поэтому данная строка кода лишняя
        self.start_number = 0
        return self
